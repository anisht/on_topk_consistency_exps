...
baseline = [0.0352028,0.059059,0.0986641,0.130444,0.13374,0.105533,0.0592153]

sum = lambda a, b: [x + y for (x,y) in zip(a,b)]

ax.set_xticks(xs)
ax.set_xticklabels(xs)
plt.plot(xs, sum(L2, baseline), label=r"$L^{(2)}$", linestyle='-.')
plt.plot(xs, sum(L3, baseline), label=r"$L^{(3)}$", linestyle='--')
plt.plot(xs, sum(L4, baseline), label=r"$L^{(4)}$", linestyle=':')
plt.plot(xs, sum(Lk, baseline), label=r"$L_k$")
plt.xlabel(r"$\alpha$")
plt.ylabel(r"Risk")
plt.legend()
plt.savefig('risk-plot-textured.png')
plt.show()
